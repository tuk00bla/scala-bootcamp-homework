object Monads {
  ...
  object State {
    implicit def stateMonad[S] = new Monad[({type x[a]=State[S, a]})#x] {
      ...
      def bind[A, B](ma: State[S, A])(amb: A => State[S, B]): State[S, B] = State(ma.run.andThen { case (s, a) => amb(a).run(s) })
    }
  }
  ...
  object Reader {
    implicit def readerMonad[R] = new Monad[({type x[a]=Reader[R, a]})#x] {
      ...
      def bind[A, B](ma: Reader[R, A])(amb: A => Reader[R, B]): Reader[R, B] = Reader((r: R) => {
        val a: A = ma.run(r)
        amb(a).run(r)
      })
    }
